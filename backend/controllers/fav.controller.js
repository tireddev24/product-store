...
export const AddToFavourites = async (req, res) => {
	...
	try {
		...
		if (isFav) {
			res.status(400).json({message: "You already added this product to favourites"});
			return;
		}
		...
		const favs = await User.findOne({_id: userId}).select("favs").populate({path: "favs"});

		res.status(201).json({success: true, message: "Added to favourites!", favs});
		return;
	} catch (error) {
		console.log(error);

		res.status(500).json({success: false, message: "Unable to add to favourites"});
		return;
	}
};
...
export const removeFromFavourites = async (req, res) => {
	...
	try {
		...
		await User.findByIdAndUpdate(isFav.favowner, {$pull: {favs: isFav._id}}, {new: true});
